...
export default function BranchesDashboard() {
    ...
    const addBranchAPI = async () => {
        try {
            ...
            const adminId = localStorage.getItem("userId"); // Admin userId from login response
            const clinicEmail = localStorage.getItem("userEmail"); // Same email used at login

            const response = await fetch(
                // "https://project01-a7ht.onrender.com/dev/v1/addBranchName",
                `${import.meta.env.VITE_API_BASE_URL}/dev/v1/addBranchName`,
                {
                    method: "POST",
                    headers: {
                    ...
                    body: JSON.stringify({
                        // email: clinicEmail, // must be same login email
            ...
            if (response.ok) {
                // Create new branch object for UI
                const branchObj = {
                    id: Date.now(),
